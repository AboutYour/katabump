...
(async () => {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        try {
            ...
            console.log('正在输入凭据...');
            try {
                // Katabump 当前登录页的实际 DOM 是：
                // <input type="email" name="email" id="email">
                // <input type="password" name="password" id="password">
                // <button id="submit" type="submit">Login</button>
                // 原来的 getByRole({ name: 'Email'/'Password' }) 在没有 label/aria-label 时容易找不到元素，
                // password 输入框尤其容易不被识别为可按名称定位的 textbox。这里改为稳定 CSS 选择器。
                await page.locator('#email, input[name="email"], input[type="email"]').first().waitFor({ state: 'visible', timeout: 15000 });
                ...
                const loginBtn = page.locator('#submit, button[type="submit"], button:has-text("Login")').first();
                ...
                // 检查登录错误
                try {
                    const errorMsg = page.locator('text=/Incorrect password|no account|invalid|错误|失败/i').first();
                    if (await errorMsg.isVisible({ timeout: 3000 })) {
                        console.error(`   >> ❌ 登录失败: 账号或密码错误/登录页返回错误提示`);
                        ...
                        await sendTelegramMessage(`❌ *${escapeMarkdown(user.username)}*\n登录失败: 账号或密码错误/登录页返回错误提示`, failScreenshot);
                        continue;
                    }
                } catch (e) { }

                if (page.url().includes('/auth/login')) {
                    console.error('   >> ❌ 登录后仍停留在登录页，可能是 Cloudflare token 未提交、账号密码错误或站点接口拦截。');
                    const failPhotoDir = path.join(process.cwd(), 'screenshots');
                    if (!fs.existsSync(failPhotoDir)) fs.mkdirSync(failPhotoDir, { recursive: true });
                    const failSafe = user.username.replace(/[^a-z0-9]/gi, '_');
                    const failScreenshot = path.join(failPhotoDir, `${failSafe}_login_still_on_login.png`);
                    try { await saveViewportScreenshot(page, failScreenshot); } catch (e) {}
                    await sendTelegramMessage(`❌ *${escapeMarkdown(user.username)}*\n登录后仍停留在登录页`, failScreenshot);
                    continue;
                }

            } catch (e) {
                console.log('登录操作遇到异常:', e.message);
                const failPhotoDir = path.join(process.cwd(), 'screenshots');
                if (!fs.existsSync(failPhotoDir)) fs.mkdirSync(failPhotoDir, { recursive: true });
                const failSafe = user.username.replace(/[^a-z0-9]/gi, '_');
                const failScreenshot = path.join(failPhotoDir, `${failSafe}_login_exception.png`);
                try { await saveViewportScreenshot(page, failScreenshot); } catch (shotError) {}
                await sendTelegramMessage(`❌ *${escapeMarkdown(user.username)}*\n登录异常: ${escapeMarkdown(e.message)}`, failScreenshot);
                continue;
            }
