...
// 直接启动 Playwright 自带的 Chromium（workflow 已通过 `npx playwright install chromium --with-deps` 安装好，
// 且依赖齐全），不再依赖系统 /usr/bin/google-chrome 与远程调试端口，避免 CI 上启动失败且无从排查。
async function launchBrowser() {
    console.log('正在启动 Playwright Chromium...');

    const launchOptions = {
        headless: false, // 配合 xvfb-run 提供真实显示，模拟有头浏览器
        args: [
            '--no-sandbox',
    ...
    try {
        ...
        throw new Error('Chrome 启动失败: ' + e.message);
    }
}
...
// --- 诊断辅助：登录/找元素失败时打印页面信息并截图 ---
async function diagnosePage(page, label) {
    console.log(`\n[诊断 ${label}]`);
    try { console.log('  当前 URL:', page.url()); } catch (e) { }
    try { console.log('  页面标题:', await page.title()); } catch (e) { }
    try {
        ...
        console.log('  页面链接/按钮:', JSON.stringify(dump, null, 2));
    } catch (e) { console.log('  无法读取页面元素:', e.message); }

    try {
        const dir = path.join(process.cwd(), 'screenshots');
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        const p = path.join(dir, `diagnose_${label}.png`);
        await page.screenshot({ path: p, fullPage: true });
        console.log('  诊断截图已保存:', p);
    } catch (e) { console.log('  截图失败:', e.message); }
}

// 兼容 "See" 可能是 link 或 button 的情况
async function findSeeLocator(page) {
    const candidates = [
        page.getByRole('link', { name: 'See' }),
        page.getByRole('button', { name: 'See' }),
        page.locator('a', { hasText: /^see$/i }),
        page.locator('button', { hasText: /^see$/i }),
    ];
    for (const loc of candidates) {
        try {
            if (await loc.first().isVisible({ timeout: 4000 })) return loc.first();
        } catch (e) { }
    }
    return null;
}
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        try {
            ...
            try {
                ...
                if (cdpClickResult) {
                    ...
                    console.log(`   >> 登录前 Turnstile 验证结果: ${turnstileVerified ? '已确认' : '未确认(仍继续尝试登录)'}`);
                } else {
        ...
        if (await errorMsg.isVisible({ timeout: 3000 })) {
          ...
          await sendTelegramMessage(`❌ *登录失败*\n用户: ${user.username}\n原因: 账号或密码错误`, failShotPath);
            ...
            await page.waitForTimeout(2000); // 等登录导航完成

            // 若登录后仍在登录页，说明登录未成功
            if (page.url().includes('login') || page.url().includes('auth')) {
                console.log('>> 登录后仍在登录页，疑似登录失败（凭据错误或 Turnstile 未通过）。');
                await diagnosePage(page, 'login_failed');
                continue;
            }

            const seeLoc = await findSeeLocator(page);
            if (!seeLoc) {
                console.log('未找到 "See" 按钮。');
                await diagnosePage(page, 'no_see');
                continue;
            }
            await seeLoc.click();
